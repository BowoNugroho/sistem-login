...
class Auth extends CI_Controller
{
	...
	public function registration()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			//token untuk di kirim ke email verify
	...
	private function _sendEmail($token, $type)
	{
		...
		$config = [
			...
			'smtp_user' => 'b946d7bc3818dc',
			'smtp_pass' => 'a5c9b5775b21c7',
			'smtp_port' => 2525,
			// 'mailtype'  => 'html',
			'charset'   => 'utf-8',
			'crlf' 		=> "\r\n",
		...
		$this->email->from('bwnh59@gmail.com', 'WEB');
	...
	public function verify()
	{
		...
		// cek ke database untuk email yg di verify
		$user = $this->db->get_where('user', ['email' => $email])->row_array();

		if ($user) {
			// cek ke database untuk token yg di verify
			$user_token = $this->db->get_where('user_token', ['token' => $token])->row_array();

			if ($user_token) {

				// batas waktu token verify
	...
	public function changePassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
			Password has been changen! Please login.</div>');
			redirect('auth');
		}
	}
}
